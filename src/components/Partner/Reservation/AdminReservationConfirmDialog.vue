    ...
    <app-card heading="파트너 객실예약 신청 데이터" custom-classes="border" content-classes="pt-2">
      <v-row>
        ...
        <v-col cols="4" md="2">
          ...
          <v-text-field :value="form.userName" dense readonly hide-details />
        </v-col>
        <v-col cols="4" md="2">
          <div class="font-weight-bold info--text body-1">이용자 핸드폰번호</div>
          <v-text-field :value="form.userTel" dense readonly hide-details />
        </v-col>
        <v-col cols="4" md="2">
          <div class="font-weight-bold info--text body-1">업체 주문번호</div>
          <v-text-field :value="form.comRsvNo" dense readonly hide-details />
        </v-col>
      </v-row>
      <v-row align="end">
        <v-col cols="4" md="2">
        ...
        <v-col cols="4" md="2" class="font-weight-bold success--text">
          ...
          <div>
            ...
            <span class="primary--text"> / 객실 수 : </span><span class="red--text">{{partner.rmCnt}}</span>
          </div>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="4" md="2">
          ...
          <v-text-field :value="form.ciYmd | date" dense readonly hide-details append-icon="today" />
        </v-col>
        <v-col cols="4" md="2">
        ...
        <v-col cols="4" md="2">
          ...
          <v-text-field :value="form.rmCnt" dense readonly hide-details />
        </v-col>
        <v-col cols="4" md="2">
  ...
  methods: {
      ...
      try {
        if (this.param.appSeq) {
          ...
          for (let index = 0; index < days; index++) {
            const value = moment(this.form.ciYmd).add(index, 'days')
    ...
    async submit () {
      try {
        ...
        const res = await partnerReservationService.processPartnerReservationApply(this.form.appSeq, this.replyForm)
        if (res.data.status === true) {
          await this.$dialog.alert('객실예약 신청을 처리 하였습니다.')
          // this.close({ change: true })
          this.search()
        } else {
          await this.$dialog.alert(`객실예약 신청 처리에 실패했습니다.<br/>오류내용: ${res.data.errMsg}`)
