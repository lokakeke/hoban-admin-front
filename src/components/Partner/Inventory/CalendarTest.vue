  ...
  <div>
    <v-row align="center" no-gutters>
      <v-col cols="4" class="text-right">
        <v-btn color="info" class="font-raise" text @click="changeDate(true)">
      ...
      <v-col cols="4" class="px-4">
        <v-menu
          ...
          <template v-slot:activator="{ on }">
            <v-btn v-on="on" block text color="info" class="font-raise">
      ...
      <v-col cols="4" class="text-left">
        <v-btn color="info" text class="font-raise" @click="changeDate(false)">
      ...
      for (let index = 0; index <= days; index++) {
        ...
        if (this.form.ptnrList) {
          ...
          for (const ptnr of this.form.ptnrList.ptnrList) {
            list.push({ partnerSeq: ptnr.partnerSeq, label: ptnr.partnerName, count: Math.floor(Math.random() * 25) })
          }
