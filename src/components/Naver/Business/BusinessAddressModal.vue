<template>
  <dialog-base :instance="instance" right :title="`주소 ${ instance.params.addrModal.commCdNo ? '수정': '등록' }`">
    <template v-slot:buttons>
      <v-btn dark text @click="emit()">
        <v-icon left>check</v-icon>{{ instance.params.addrModal.commCdNo ? '수정': '등록' }}
      </v-btn>
    </template>
        ...
        <v-row>
          <v-col cols="12">
            ...
            <v-text-field placeholder="주소이름을 입력해주세요." counter="30"
              v-model="form.commCdNm" :rules="betweenLengthRules(1,30).concat(emptyRules)"></v-text-field>
          </v-col>
          <v-col cols="10">
          ...
          <v-col cols="12">
            ...
            <v-textarea outlined dense class="mt-2"
              v-model="form.commCdDesc" :value="form.commCdDesc"
  ...
  methods: {
    ...
    emit () {
      this.validForm(this.$refs.form).then(() => {
        this.$dialog.confirm(this.form.commCdNo ? '주소를 수정하시겠습니까?' : '주소를 입력하시겠습니까?').then(() => {
