  ...
  <v-form>
    <table border="1" style="border:1px solid #ddd;" class="text-center">
      ...
      <tr v-if="item.pkgYn === 'Y'">
        <th>패키지명</th>
        <td>{{ item.pkgNm }}({{ item.mid }})</td>
        <td>{{ form.ptnrNm ? form.ptnrNm : '-' }}({{ dgnsInfo.memNo ? dgnsInfo.memNo : '-' }})</td>
      </tr>
      <tr v-else>
        <th>회원번호</th>
        <td>{{ item.mid }}</td>
      ...
      <tr>
        <th>영업장명</th>
        <td>{{ item.storeNm }} ({{ item.storeCd }})</td>
        <td>{{ dgnsInfo.storeNm ? dgnsInfo.storeNm : '-' }} ({{ dgnsInfo.storeCd ? dgnsInfo.storeCd : '-' }})</td>
      </tr>
      <tr>
        <th>객실타입명</th>
        <td>{{ form.naver.dmItemName }}</td>
        <td>{{ dgnsInfo.rmTypeNm ? dgnsInfo.rmTypeNm : '-' }} ({{ dgnsInfo.rmTypeCd ? dgnsInfo.rmTypeCd : '-' }})</td>
      </tr>
      <tr>
        <th>블럭코드</th>
        <td>{{ item.rsvBlckCd }}</td>
        <td>{{ dgnsInfo.rsvBlckCd ? dgnsInfo.rsvBlckCd : '-' }}</td>
      </tr>
      <tr>
        <th>DGNS예약부가정보</th>
        ...
        <td colspan="2" class="pb-1" v-else>
          ...
          <v-chip class="mr-1 mt-1" small>등록자: {{ dgnsInfo.crtNm }}</v-chip>
          <v-chip class="mr-1 mt-1" small>등록일시: {{ dgnsInfo.crtDt }}</v-chip>
          <v-chip class="mr-1 mt-1" small>수정자: {{ dgnsInfo.updNm }}</v-chip>
          <v-chip class="mr-1 mt-1" small>수정일시: {{ dgnsInfo.updDt }}</v-chip>
          <v-chip class="mr-1 mt-1" small>취소사유코드: {{ dgnsInfo.cancelResnCd ? dgnsInfo.cancelResnCd : '(없음)' }}</v-chip>
        </td>
      </tr>
    </table>
  </v-form>
