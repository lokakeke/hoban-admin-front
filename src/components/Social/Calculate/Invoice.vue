      ...
      <v-container fluid>
        ...
        <v-row dense>
          ...
          <v-col cols="9" class="body-1 black--text py-2 pl-3 font-weight-bold">
            {{totalRow.totCmsnAmt + addInvoiceAmtSum | price}} 원
          </v-col>
          ...
          <v-col cols="12" class="mb-1">
            <v-simple-table dense class="bordered text-center" v-dragscroll="{target: '.v-data-table__wrapper'}">
              ...
              <tr>
                ...
                <th class="text-center">미사용 수량</th>
                <th class="text-center">미사용</th>
                <th class="text-center">미사용 수수료</th>
              ...
              <tr v-for="(item, index) of form.invoiceList" :key="index" @click="detailInfo(item)" class="pointer">
                ...
                <td>{{item.uuseQty | price}}</td>
                <td>{{item.bkpmsAmt | price}} <span v-if="item.bkpmsAmt != 0">({{item.bkpmsRate}}%)</span></td>
                <td>{{item.uuseCmsnAmt | price}} <span v-if="item.uuseCmsnAmt != 0">({{item.uuseCmsnRate}}%)</span></td>
                <td>{{item.totCmsnAmt | price}}</td>
              </tr>
              <!-- 추가정산 -->
              ...
              <tr v-if="addInvoiceList.length > 0">
                <td colspan="8" class="text-right font-weight-bold">정산 원금</td>
                <td>{{totalRow.totCmsnAmt | price}}</td>
                <td class="font-weight-bold">추가정산 합계</td>
                <td>{{addInvoiceAmtSum | price}}</td>
                <td class="font-weight-bold">정산원금 + 추가정산 합계</td>
                <td>{{(totalRow.totCmsnAmt + addInvoiceAmtSum) | price}}</td>
              </tr>
              <tr class="sum-row">
                <td colspan="5">합 계</td>
                ...
                <td>{{totalRow.uuseQty | price}}</td>
                <td>{{totalRow.bkpmsAmt | price}}</td>
                <td>{{totalRow.uuseCmsnAmt | price}}</td>
                <td>{{totalRow.totCmsnAmt + addInvoiceAmtSum | price}}</td>
              </tr>
              </tbody>
              <tbody v-else>
        ...
        <span id="printMe" v-show="false">
          <v-row dense class="invoiceTop">
            ...
            <v-col cols="9" class="body-1 black--text py-2 pl-3 font-weight-bold">
              <!--<span class="invoiceContent">{{form.deposit | price}} 원</span>-->
              <span class="invoiceContent">{{totalRow.totCmsnAmt + addInvoiceAmtSum | price}} 원</span>
            </v-col>
...
export default {
  ...
  data () {
    return {
      ...
      addInvoice: [
        { text: '우대번호', value: 'ticketNo', align: 'center', sortable: false, width: '10%' },
        { text: '우대번호 순번', value: 'ticketSeq', align: 'center', sortable: false, width: '10%' },
        { text: '우대번호 상태', value: 'checkTicket', align: 'center', sortable: false, width: '10%' },
        { text: '사용수량', value: 'useQty', align: 'center', sortable: false, width: '10%' },
        { text: '금액', value: 'useAmt', align: 'center', sortable: false, width: '10%' },
        { text: '상태', value: 'itemStat', align: 'center', sortable: false, width: '10%' },
  ...
  methods: {
    ...
    setCalculateInfo () {
      ...
      this.form.taxInvoice = this.totalRow.cmsnAmt + this.totalRow.uuseCmsnAmt
      // 입금금액
      this.form.deposit = this.totalRow.totCmsnAmt
      // 정산기간
