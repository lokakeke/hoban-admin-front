      ...
      <v-expand-transition>
        <div v-show="tableShow" class="text-right">
          <v-simple-table v-dragscroll="{target: '.v-data-table__wrapper'}"
            ...
            <thead>
            <tr>
              <th id="channel_firstCol" class="text-center sticky" style="left: 0">채널사</th>
...
export default {
  ...
  methods: {
    async search () {
      try {
        console.log('fire : channel')
        const data = await statisticsService.selectSocialChannelStatistics(this.searchCondition)
        this.data = data
    ...
    observeSize () {
      ...
      if (document.getElementById('channel_firstCol')) {
        observerTable.observe(document.getElementById('channel_firstCol'))
      }
      this.observerTable = observerTable
    },
    exportExcel () {
      this.downLoadExcelPost('/api/social/statistics/channelExcel',
        `소셜 채널별 ${this.searchCondition.saleKind === 'S' ? '판매' : this.searchCondition.saleKind === 'U' ? '사용' : '취소'} 통계`, this.searchCondition)
    }
  },
  /**
