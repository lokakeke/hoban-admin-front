      ...
      <v-expand-transition>
        <div v-show="tableShow" class="text-right">
          <v-simple-table v-dragscroll="{target: '.v-data-table__wrapper'}"
            ...
            <thead>
            <tr>
              <th id="partner_firstCol" class="text-center sticky" style="left: 0">파트너</th>
            ...
            <tr v-for="(row, index) of list" :key="index">
              <td v-if="row.name" rowspan="2" class="text-center sticky" style="left: 0" >{{row.name}}</td>
...
export default {
  ...
  methods: {
    async search () {
      try {
        console.log('fire : partner')
        const data = await statisticsService.selectSocialPartnerStatistics(this.searchCondition)
        this.data = data
    ...
    observeSize () {
      ...
      if (document.getElementById('partner_firstCol')) {
        observerTable.observe(document.getElementById('partner_firstCol'))
      }
      this.observerTable = observerTable
    },
    exportExcel () {
      this.downLoadExcelPost('/api/social/statistics/partnerExcel',
        `소셜 파트너별 ${this.searchCondition.saleKind === 'S' ? '판매' : this.searchCondition.saleKind === 'U' ? '사용' : '취소'} 통계`, this.searchCondition)
    }
  },
  /**
