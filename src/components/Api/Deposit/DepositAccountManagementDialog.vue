    ...
    <v-form ref="form" lazy-validation v-if="form">
      <v-row>
        ...
        <v-col sm="3" md="2">
          ...
          <v-text-field :value="form.ptnrNm" label readonly hide-details v-else></v-text-field>
        </v-col>
...
export default {
  name: 'DepositAccountManagementDialog',
  extends: DialogBase,
  data () {
    return {
      // 폼
      ...
      // 파트너 기간 헤더
      headers: [
        { text: '예치금 구분', value: 'taskTypeCdNm', align: 'center' },
    ...
    async init () {
      try {
        ...
        if (this.isPartner === true) {
          ...
          this.form.ptnrNm = this.user.name
        }
        ...
        options: {
          ...
          closeCallback: params => {
            if (params && params.data) {
              this.$set(this.form, 'ptnrNm', params.data.ptnrNm)
