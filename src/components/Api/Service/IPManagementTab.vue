      ...
      <v-row>
        ...
        <v-col cols="12">
          <v-data-table :no-data-text="'등록된 IP 정보가 없습니다.'" :headers="headers" :items="list" disable-sort disable-pagination hide-default-footer
            ...
            <template v-slot:item.serverCd="{item}">
              {{item.serverCd | label(serverCdList, 'commCd', 'commCdNm')}}
            </template>
  ...
  mounted () {
    ...
    if (this.isPartner) {
      this.headers = [
        ...
        { text: 'IP 명', value: 'ipNm', align: 'center' },
        { text: '서버 구분', value: 'serverCd', align: 'center' },
  ...
  methods: {
    ...
    addRow (row) {
      if (row) {
        ...
        this.form = { useIp: '', ipNm: '', serverCd: '', useYn: 'Y' }
        this.isNewTab = true
      }
      // dialog open
      this.$store.dispatch('dialog/open', {
        componentPath: '/Api/Service/IPManagementAddDialog',
        params: {
          isNewTab: this.isNewTab,
          form: this.form,
          serverCdList: this.serverCdList,
          list: this.list,
          index: this.index
        },
        options: {
      ...
      this.validForm(this.$refs.form).then(() => {
        if (this.isNew) {
          this.$dialog.confirm('API 서비스를 등록 하시겠습니까?').then(() => {
            ...
            serviceManagementService.insertServiceManagement(data).then(() => {
