    ...
    <v-form ref="form" lazy-validation autocomplete="off">
      <v-row class="elevation-2">
        ...
        <v-col cols="3">
          ...
          <v-text-field :value="form.memberNo" label="" readonly hide-details></v-text-field>
        </v-col>
<!--        <v-col cols="3">
          <v-label>대매사(대매사 코드)</v-label>
          <v-text-field :value="form.agentCode ? form.agentCodeName + '  (' +form.agentCode+')' : '' " label="" readonly
                        hide-details></v-text-field>
        </v-col>-->
        <v-col cols="3">
          <v-label>예치금 KEY</v-label>
          <v-text-field :value="form.depositKey" label="" readonly hide-details></v-text-field>
        </v-col>
        <v-col cols="3">
          <v-label>예치금 적용여부</v-label>
          <v-text-field :value="form.depositYn === 'Y'? '적용': form.depositYn === 'N'? '미적용': ''" label="" readonly
                        hide-details></v-text-field>
        </v-col>
        <v-col cols="6">
          <v-label>예치금 미적용사유</v-label>
          <v-textarea :value="form.depositDesc" auto-grow rows="1" label="" readonly></v-textarea>
        </v-col>
        <v-col cols="3">
          <v-label>이용 시작기간</v-label>
          <v-text-field :value="form.useStartDate | date" label="" readonly append-icon="event_note"
                        hide-details></v-text-field>
        </v-col>
        <v-col cols="3">
          <v-label>이용 종료기간</v-label>
          <v-text-field :value="form.useEndDate | date" label="" readonly append-icon="event_note"
                        hide-details></v-text-field>
        </v-col>
      </v-row>
        ...
        <v-col cols="3">
          ...
          <v-text-field v-if="isNew" v-model="form.businessId" :rules="emptyRules" required>
            <template v-slot:append-outer>
              ...
              <v-btn outlined color="primary" tabindex="-1" @click="checkBusinessID">
        ...
        <template v-if="!isNew">
          <v-col cols="12" v-if="!isNew">
            ...
            <v-textarea v-model="form.tokenKey" readonly rows="1" auto-grow id="tokenArea">
              <template v-slot:append-outer>
                <v-btn color="primary" @click="copyToken">
                  <v-icon left>folder_open</v-icon>
                  복사
                </v-btn>
                <v-btn color="accent" @click="createToken">
          ...
          <v-col cols="12">
            ...
            <v-simple-table class="bordered mt-5">
              ...
              <tbody>
              <tr v-for="term in tokenHistory" :key="term.historySeq">
                <td class="text-center">{{ term.historySeq }}</td>
                <td :class="['text-center', term.tokenKey !== form.tokenKey ? 'strike' : '']">
                  {{ term.tokenKey }}
                </td>
                <td class="text-center">{{ term.tokenKey === form.tokenKey ? '사용중' : '폐기' }}</td>
              </tr>
              <tr v-if="!tokenHistory || tokenHistory.length === 0">
    ...
    <v-row v-if="writeAuth">
      <v-col cols="12" class="text-right">
        <v-btn v-if="isNew" color="info" rounded @click="submit">
          <v-icon left>arrow_forward</v-icon>
          다음 단계
        </v-btn>
        <v-btn v-else color="info" rounded @click="submit">
          <v-icon left>check</v-icon>
          수정
        </v-btn>
      </v-col>
    ...
    createToken () {
      this.$dialog.confirm('토큰 재발행 시 기존 토큰은 사용하실 수 없습니다.<br/>재발행을 진행하시겠습니까?').then(() => {
        service.createTokenKey(this.form.businessId).then(res => {
          ...
          this.tokenHistory = res.data.tokenHistory
