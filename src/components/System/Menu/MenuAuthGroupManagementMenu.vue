...
export default {
  extends: DialogBase,
  name: 'menuAuthGroupManagementMenu',
  data () {
    return {
      menuList: [],
      selectMenu: [],
  ...
  methods: {
    ...
    open () {
      if (this.selectMenu && this.selectMenu.length > 0) {
        ...
        this.menuWrite = this.getSelectMenuList(this.menuList, false) // _.map(_.filter(this.selectMenu.slice(), { writeAuthYn: 'Y' }), 'menuId')
      } else {
        this.myMenu = []
        this.menuWrite = []
      }
    ...
    setMenu () {
      this.$dialog.confirm('메뉴권한그룹(' + this.groupName + ')의<br/>메뉴를 적용 하시겠습니까?').then(() => {
        ...
        menuAuthService.updateMenuAuth({ grupId: this.grupId, menuAuthList: array }).then(res => {
