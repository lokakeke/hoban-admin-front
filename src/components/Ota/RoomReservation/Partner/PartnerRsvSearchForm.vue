  ...
  <div :key="key">
    ...
    <v-row no-gutters justify="space-between">
      ...
      <v-col align-self="center" class="text-right pa-0">
        <v-btn v-if="!isMultiCancel" outlined rounded color="blue" @click="makeNew('ROOM')">
          <v-icon left>edit</v-icon>객실신규(Ctrl+F5)
        </v-btn>
        <v-btn v-if="!isMultiCancel" outlined rounded color="blue" @click="makeNew('PKG')">
          <v-icon left>edit</v-icon>패키지신규(Ctrl+F6)
        </v-btn>
...
export default {
  ...
  methods: {
     ...
     */
    setPartnerInfo () {
      this.$set(this.form, 'memNo', this.partnerInfo.memNo)
      this.$set(this.form, 'memNm', this.partnerInfo.memNm)
      this.$set(this.form, 'agentCd', this.partnerInfo.agentCd)
      this.$set(this.form, 'agentNm', this.partnerInfo.agentNm)
    },
    /**
     * 파트너 정보 세팅
     */
    ...
    emit (isRefresh) {
      this.validForm(this.$refs.form).then(() => {
        if (isRefresh === true) { // 초기화
          ...
          this.form.rsvNo = ''
          this.form.keyRsvNo = ''
          if (this.roomType.value === 'OTA_ROOM_API') { // 객실인 경우에만 agentCd 초기화
            this.form.agentCd = ''
          }
          this.form.storeCd = ''
          this.form.storeNm = ''
     ...
     */
    makeNew (roomType) {
      this.$emit('partner-new-rsv', roomType)
    },
    /**
     * 엑셀 다운로드
     */
