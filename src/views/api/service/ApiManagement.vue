      ...
      <v-row>
        ...
        <v-col v-else cols="12" class="py-0">
          <template v-for="item of apiList">
            <v-hover v-slot:default="{ hover }" :key="item.commCd">
              <v-card raised
    ...
    <app-card :heading="'API 서비스 등록 업체목록'" col-classes="col-12 col-md-6" auto-height>
      <template v-slot:items v-if="selectService">
        <v-row justify="center" class="py-2 subtitle-1 font-weight-black border-bottom">
          {{taskType | label(taskTypeList, 'commCd', 'commCdNm')}} - {{selectService.commCdNm}} ({{selectService.commCd}})
        </v-row>
      </template>
      <v-row>
        ...
        <v-col v-else cols="12">
          ...
          <v-simple-table>
            ...
            <tbody>
              <tr v-for="item of serviceOpenList" :key="item.ptnrNo + '_' + item.businessId">
...
export default {
    ...
    apiList () {
      if (this.searchKeyword) {
        return this.apiOriginList.filter(data =>
          data.commCdNm.indexOf(this.searchKeyword) > -1 ||
  ...
  methods: {
    ...
    async showItem (item) {
      try {
        if (!this.apiOriginList.find(data => data.commCd === item.commCd).active) {
          const res = await serviceOpenService.selectServiceOpenFullList(this.taskType, item.commCd)
    ...
    async refresh () {
      const res = await serviceOpenService.selectServiceOpenFullList(this.taskType, this.selectService.commCd)
      this.serviceOpenList = res.data
    },
    async submit () {
      try {
