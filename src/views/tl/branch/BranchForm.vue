<template>
  <div>
    ...
    <v-text-field v-model="formData.brcName" :counter="100" label="" :rules="emptyRules" required class="pt-0 ime-active"></v-text-field>
    <v-label>영업장</v-label>
    ...
    <v-text-field v-model="formData.tlLoginPw" type="password" autocomplete="new-password" label="" :rules="emptyRules" class="pt-0"></v-text-field>

    <v-checkbox class="mt-0" v-model="formData.useYn" label="사용여부" required true-value="Y" false-value="N"></v-checkbox>
    <v-checkbox class="mt-0" v-model="formData.autoRsvYn" label="자동예약 일시정지(일시정지 해제 시 일시정지 기간에 쌓인 예약은 다시 자동으로 들어갑니다.)" required true-value="N" false-value="Y"></v-checkbox>
