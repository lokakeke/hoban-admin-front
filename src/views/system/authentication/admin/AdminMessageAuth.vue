                ...
                <template v-slot:items>
                    <v-row>
                        <v-col cols="12" class="pb-0">
                            <v-text-field v-model="searchMenu" hide-details single-line outlined small dense
                                          append-icon="mdi-magnify" label="Search" clearable/>
                        </v-col>
                    </v-row>
                </template>
                <v-list dense v-if="groupFilterList && groupFilterList.length > 0">
                    <v-list-item v-for="item of groupFilterList" :key="item.menuAuthGroupId"
                                 @click="viewDetail(item, true)" class="menu-list" :class="item.active? 'active' : ''">
                        <v-list-item-action>
                    ...
                    <template v-if="selectedGroup && selectedGroup.menuAuthGroupId">
                        <v-row>
                            ...
                            <v-col cols="6">
                                ...
                                <v-text-field v-model="selectedGroup.menuAuthGroupName" disabled :rules="emptyRules"
                                              label="" class="pt-0"/>
                            </v-col>
                            <v-col class="py-0">
                                <v-checkbox class="mt-0" v-model="selectedGroup.useYn" disabled label="사용여부"
                                            hide-details true-value="Y" false-value="N"/>
                            </v-col>
                        </v-row>
                    </template>
                    <v-row v-else align="center" justify="center">
            ...
            <app-card :heading="'메시지 권한 관리자 리스트'">
                <template v-if="selectedGroup">
                    <v-row no-gutters justify="end" class="mb-2">
                        <v-col cols="6">
                            <v-text-field v-model="search" hide-details single-line outlined small dense
                                          append-icon="mdi-magnify" label="Search"/>
                        </v-col>
                    </v-row>
                    <v-data-table :no-data-text="menuAuthGroupName +' 그룹에 관리자 정보가 없습니다.'"
                                  :no-results-text="'검색 결과가 없습니다.'" :headers="headers"
                                  :items="selectedGroup.adminAccountList" :search="search">
                ...
                <template v-slot:items v-if="selectedGroup">
                    <v-row>
                        <v-col cols="6" class="pb-0">
                            <v-text-field v-model="searchAuth" hide-details single-line outlined small dense
                                          append-icon="mdi-magnify" label="Search" clearable/>
                        </v-col>
                        <v-col cols="6" class="pb-0">
                            ...
                            <template v-if="selectedGroup.menuAuthGroupId && selectedGroup.messageAuths.length >0">
                                <v-list dense>
                                    <v-list-item v-for="(messageAuth,index) of selectedGroup.messageAuths" :key="index">
                                        ...
                                        <v-list-item-content>
                                            <v-list-item-title>
                                                {{ messageAuth.messageName }} <span v-for="n in 0" :key="n">{{
                                                    messageAuth.messageName
                                                }}</span> ({{ messageAuth.messageId }})
                                            </v-list-item-title>
                                        </v-list-item-content>
                                    </v-list-item>
                                </v-list>
...
<script>
import adminMessageAuth from '@/api/modules/system/authentication/admin/adminMessageAuth.service'
import commonCodeService from '@/api/modules/system/commonCode.service'
import messageService from '@/api/modules/system/message.service'

export default {
  name: 'adminMessageAuth',
  mounted () {
  ...
  methods: {
    ...
    load () {
      this.dialog = false
      adminMessageAuth.selectAllGroupByMenuAuthGroup().then(res => {
