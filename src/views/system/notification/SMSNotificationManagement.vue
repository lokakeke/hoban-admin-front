      ...
      <app-card :heading="'상세 정보'" col-classes="col-sm-12 col-md-6">
        ...
        <template v-else>
          <v-form ref="detail" lazy-validation>
            ...
            <v-autocomplete v-model="detail.svcNo" :items="serviceList" :item-value="'commCd'" :item-text="'commCdNm'" autocomplete="off" :rules="emptyRules" label required class="pt-0"></v-autocomplete>
            <v-label>진행단계</v-label>
            <v-autocomplete v-model="detail.progStat" :items="progStatList" :item-value="'commCd'" :item-text="'commCdNm'" autocomplete="off" :rules="emptyRules" label required class="pt-0"></v-autocomplete>
            ...
            <v-text-field v-model="detail.notifyNm" :rules="emptyRules" label required class="pt-0"/>
            ...
            <v-data-table
              ...
              hide-default-footer
            >
              <template v-slot:item.buttonNm="{item}">
                <v-text-field v-model="item.buttonNm" label="" :rules="emptyRules"></v-text-field>
              </template>
    ...
    <v-navigation-drawer fixed v-model="dialog" :width="600" right app stateless temporary>
      ...
      <v-container fluid>
        <v-form ref="form" lazy-validation>
          <v-label>서비스 번호</v-label>
          <v-autocomplete v-model="form.svcNo" :items="serviceList" :item-value="'commCd'" :item-text="'commCdNm'" autocomplete="off" :rules="emptyRules" label required class="pt-0"></v-autocomplete>
          <v-label>진행단계</v-label>
          <v-autocomplete v-model="form.progStat" :items="progStatList" :item-value="'commCd'" :item-text="'commCdNm'" autocomplete="off" :rules="emptyRules" label required class="pt-0"></v-autocomplete>
          <v-label>템플릿 번호</v-label>
...
<script>
import smsManageService from 'Api/modules/system/smsNotiManagement.service'
...
export default {
  ...
  data () {
    return {
      ...
      buttonList: [],
      buttonheaders: [
        { text: '버튼명', value: 'buttonNm', align: 'center', sortable: false },
  ...
  methods: {
    ...
    // 버튼 추가
    addButton () {
      this.buttonList.push({ buttonNm: '', url: '' })
    },
    // 버튼 삭제
    removeButton (data) {
