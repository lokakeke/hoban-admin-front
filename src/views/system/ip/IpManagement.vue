        ...
        <template v-if="ipFilterList && ipFilterList.length > 0">
          <v-list dense>
            ...
            >
              ...
              <v-list-item-content>
                <v-list-item-title :class="ip.useYn === 'N'? 'strike' : ''">{{ ip.ipNm }} ({{ ip.useIp }} / {{ ip.emplNo }})</v-list-item-title>
              </v-list-item-content>
              <v-list-item-action>
      ...
      <app-card :heading="'상세 정보'" col-classes="col-sm-12 col-md-6">
        ...
        <template v-else>
          <v-form ref="ipInfo" lazy-validation>
            ...
            <v-text-field v-model="ipInfo.ipNm" :rules="emptyRules" label required class="pt-0"></v-text-field>
            <v-label>사번</v-label>
            <v-text-field v-model="ipInfo.emplNo" label required class="pt-0"></v-text-field>
            <v-label>서버 구분</v-label>
            <v-autocomplete v-model="ipInfo.serverCd" :rules="emptyRules" :items="serverCodeList" :item-text="'commCdNm'" :item-value="'commCd'" required class="pt-0"></v-autocomplete>
            <v-checkbox
              class="mt-0"
    ...
    <v-navigation-drawer fixed v-model="dialog" :width="600" right app stateless temporary>
      ...
      <v-container fluid>
        <v-form ref="form" lazy-validation>
          ...
          <v-label>IP 이름</v-label>
          <v-text-field v-model="form.ipNm" :rules="emptyRules" label required class="pt-0"></v-text-field>
          <v-label>사번</v-label>
          <v-text-field v-model="form.emplNo" label required class="pt-0"></v-text-field>
          <v-label>서버 구분</v-label>
          <v-autocomplete v-model="form.serverCd" :rules="emptyRules" :items="serverCodeList" :item-text="'commCdNm'" :item-value="'commCd'" required class="pt-0"></v-autocomplete>
          <v-checkbox
            class="mt-0"
            v-model="form.useYn"
            label="사용여부"
...
<script>
import ipManageService from 'Api/modules/system/ipManagement.service'
  ...
  computed: {
    ipFilterList () {
      if (this.ipList && this.ipList.length > 0) {
        if (this.filterText) {
          return this.ipList.filter(data => data.ipNm.indexOf(this.filterText) !== -1 || data.useIp.indexOf(this.filterText) !== -1 || (data.emplNo || '').indexOf(this.filterText) !== -1)
        } else {
          return this.ipList
        }
      } else {
      ...
      this.validForm(this.$refs.ipInfo).then(() => {
        this.$dialog
          ...
          .then(() => {
            ...
            newInfo.ipNm = this.ipInfo.ipNm
            newInfo.useYn = this.ipInfo.useYn
            newInfo.emplNo = this.ipInfo.emplNo
            newInfo.serverCd = this.ipInfo.serverCd
