        ...
        <transition name="slide-fade" mode="out-in">
          <template v-if="selectGroup">
            <v-form ref="form" lazy-validation :key="selectGroup" autocomplete="off">
              <menu-auth-management-form :formData.sync="form"></menu-auth-management-form>
            </v-form>
          </template>
          <v-row v-else align="center" justify="center">
            메뉴권한 그룹을 선택해 주세요.
      ...
      <v-container fluid>
        <v-form ref="addForm" lazy-validation autocomplete="off" onsubmit="return false;">
          <div @keypress.enter="commit(false)">
            <menu-auth-management-form :formData.sync="addForm"></menu-auth-management-form>
          </div>
...
<script>
import MenuAuthManagementForm from 'Components/System/Menu/MenuAuthGroupManagementForm.vue'
import menuAuthService from 'Api/modules/system/menuAuth.service'
import menuService from 'Api/modules/system/menu.service'

export default {
  components: { MenuAuthManagementForm },
  name: 'menuAuthGroupManagement',
  mounted () {
  ...
  computed: {
    groupName () {
      ...
      if (this.selectGroup) {
        ...
        text = select ? select.grupNm : ''
      }
      return text
    },
    groupFilterList () {
      return this.groupList.filter(data => this.searchMenu ? data.grupNm.indexOf(this.searchMenu) > -1 : true)
    },
    groupMenuFilterList () {
      return this.menuList.filter(data => this.searchAuth ? data.menuNm.indexOf(this.searchAuth) > -1 : true)
    }
  },
  data () {
    return {
  ...
  methods: {
    openMenu () {
      this.$store.dispatch('dialog/open', {
        componentPath: '/System/Menu/MenuAuthGroupManagementMenu',
        params: {
