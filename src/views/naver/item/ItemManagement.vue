  ...
  <v-row wrap>
    <app-card :heading="'객실 목록'" col-classes="col-12">
      ...
      <v-row>
        ...
        <v-col cols="3">
          <v-text-field v-model="rmTypeCd" append-icon="search" clearable label="객실타입코드" hide-details @input="searchList"></v-text-field>
        </v-col>
        <v-col cols="2">
      ...
      >
        ...
        <template v-slot:item.pkgInfo="{item}">
          <div class="text-left pa-1" v-if="item.pkgYn === 'Y'" @click="showPackageInfoModal(item)" style="cursor: pointer;">
            <p class="pa-1 ma-0">패키지명 : {{ item.pkgNm }} ({{ item.mid }})</p>
            <p class="pa-1 ma-0">영업장명 : {{ item.storeNm }} ({{ item.storeCd }})</p>
            <p class="pa-1 ma-0">객실타입명 : {{ item.rmTypeNm }} ({{ item.rmTypeCd }})</p>
            <p class="pa-1 ma-0">시작/종료일 : {{ moment(item.pkgBgnYmd).format('YYYY.MM.DD') }} ~ {{ moment(item.pkgEndYmd).format('YYYY.MM.DD') }}</p>
            <p class="pa-1 ma-0">블럭코드 : {{ item.rsvBlckCd }}</p>
          </div>
...
export default {
  ...
  methods: {
    ...
    searchList () {
      this.mid = this.mid || ''
      this.rmTypeCd = this.rmTypeCd || ''
      this.list = _.cloneDeep(this.originList).filter((item) => {
        item.mid = item.mid || ''
        item.rmTypeCd = item.rmTypeCd || ''
        item.sendYn = item.sendYn || ''
        item.pkgYn = item.pkgYn || ''
        if (this.useYn === 'Y' || this.useYn === 'N') {
          return item.isImp === this.useYn && item.mid.indexOf(this.mid) > -1 && item.rmTypeCd.indexOf(this.rmTypeCd) > -1
        } else {
          return item.mid.indexOf(this.mid) > -1 && item.rmTypeCd.indexOf(this.rmTypeCd) > -1
        }
      }).filter((item) => {
        if (this.sendYn) {
    ...
    addItemRoom () {
      this.$store.dispatch('dialog/open', {
        componentPath: '/Naver/Item/ItemManagementRoomForm',
        params: {
    ...
    addItemPackage () {
      this.$store.dispatch('dialog/open', {
        componentPath: '/Naver/Item/ItemManagementPackageForm',
        params: {
          dmStoreId: this.searchParam.dmStoreId
        },
        options: {
    ...
    showDetail (item) {
      ...
      this.$store.dispatch('dialog/open', {
        componentPath: '/Naver/Item/ItemManagementView',
        params: {
