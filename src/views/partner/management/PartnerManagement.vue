  ...
  computed: {
    ...
    searchList () {
      const searchList = [
        ...
        { key: 'menuAuthGrupId', label: '메뉴 권한', type: 'select', list: this.menuAuthList, listValue: 'grupId', listText: 'grupNm' },
        {
          key: 'lockYn',
          label: '잠김 여부',
          type: 'select',
  ...
  methods: {
    ...
    open (rowData) {
      ...
      if (rowData) {
        ...
        partnerName = rowData.ptnrNm
      }
      // dialog open
      this.$store.dispatch('dialog/open', {
        componentPath: '/Partner/Management/PartnerManagementDialog',
        params: {
          isModify,
          partnerNo,
          partnerName,
    ...
    passwordReset (row) {
      this.$dialog.confirm(`${row.ptnrNm} 의 임시 비밀번호 발급을 진행하시겠습니까?`).then(() => {
    ...
    exportExcel () {
      this.downLoadExcel('/api/system/partnerManagement/excel', '파트너 리스트', this.searchParam)
    }
  }
}
</script>
