...
export default {
  ...
  computed: {
    ...
    searchList () {
      return [
        ...
        { key: 'pkgNm', label: '패키지명', type: 'text' },
  ...
  data () {
    return {
      ...
      headers: [
        ...
        { text: '패키지명', value: 'pkgNm', align: 'center', sortable: true },
  ...
  methods: {
    ...
    // 패키지 메뉴 출력
    showMenu (items) {
      return items.map((item) => item.pkgMenuNm).join(', ')
    },
