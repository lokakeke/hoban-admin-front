                  ...
                  >
                    <v-card hover @click="showDetail(item)">
                      ...
                      <v-card-actions class="justify-content-end">
                        {{ /* calcInd = 정산유형, aprlCd = 정산상태 | TPNE 의 경우 관리자 승인만으로 정산 마감 */ }}
                        <template v-if="writeAuth && !isPartner">
                          <template v-if="item.calcInd !== '3' && item.aprlCd === aprlCd.calculation">
                          ...
                          <template v-else-if="item.calcInd !== '3' && item.aprlCd === aprlCd.manager">
                            <v-btn color="deep-purple"
                                   dark
                                   rounded
                          ...
                          <template v-else-if="(item.calcInd === '3' && item.aprlCd === aprlCd.calculation) || item.aprlCd === aprlCd.partner">
                            <v-btn color="red"
                                   dark
                                   rounded
  ...
  methods: {
    ...
    async completeCalcForManager (item) {
      try {
        ...
        const response = await calculationService.modifyMonthCalcHistoryStatus(item.calcSeq, this.aprlCd.manager)
        const data = response.data
    ...
    async completeCalc (item) {
      try {
        ...
        const response = await calculationService.modifyMonthCalcHistoryStatus(item.calcSeq, this.aprlCd.finish)
        const data = response.data
