  ...
  methods: {
    ...
    async changeRmList (param) {
      ...
      if (
        ...
        const newParam = {}
        newParam.storeCd = param.storeCd
        newParam.startDate = param.startDate
        newParam.endDate = param.endDate
        // 잔여 객실 현황 조회
        const res = await productService.selectLeaveRooms(param.rsvBlckCd, newParam)
        // 조회된 정보 중 날짜만 뽑아서 가공
      ...
      if (item.keyRsvNo) {
        ...
        if (res.data) {
          ...
          if (this.rsvDetail.rsvType === 'PKG') { // 패키지인 경우
            ...
            this.rsvDetail.pkgNm = res.data.memNm
            this.rsvDetail.memNo = ''
            this.rsvDetail.memNm = ''
