...
import DepositManagementDialog from 'Views/social/caclulate/DepositManagementDialog'
...
export default {
  ...
  data: function () {
    return {
      ...
      partnerList: [],
      partnerHeaders: [
        { text: '파트너명', value: 'partnerNm', align: 'center', sortable: false },
  ...
  methods: {
    ...
    getDepositList (event) {
      ...
      this.partnerNm = event.partnerNm
  ...
  computed: {
    searchList () {
      return [
        { key: 'partnerNm', label: '파트너명', type: 'text' }
      ]
    },
    subheader () {
      return '예치금 등록관리 - ' + this.partnerNm
    }
  }
}
</script>
