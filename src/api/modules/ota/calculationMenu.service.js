...
const calculationMenuService = {
   ...
   */
  selectCalcMenuList () {
    return api.get('/api/system/ota/calculation/menu/list')
  },
  /**
   * 정산 > 메뉴 조회
  ...
  selectCalcMenu (partnerSeq) {
    return api.get(`/api/system/ota/calculation/menu/${partnerSeq}`)
  },
  /**
   * 정산 > 메뉴 목록 열기/닫기
   * @param partnerSeq 파트너 번호
  ...
  toggleCalcMenu (partnerSeq, calcMenuCode, openYn) {
    return api.post(`/api/system/ota/calculation/menu/toggle/${partnerSeq}/${calcMenuCode}/${openYn}`)
  }
}
